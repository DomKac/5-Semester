...
exp:
  NUM               { $$ = ($1 % P); number[0] = '/0'; sprintf(number, "%d", ($1 % P)); strcat(number, " "); strcat(str, number); }
...
| '-' exp %prec NEG { $$ = negP($2, P); }
| exp '^' powexp    { $$ = powP($1, $3, P); strcat(str, "^ "); }
| '(' exp ')'       { $$ = $2; }
;

powexp:
  NUM               { $$ = ($1 % (P-1));  number[0] = '/0'; sprintf(number, "%d", ($1 % (P-1))); strcat(number, " "); strcat(str, number); }
